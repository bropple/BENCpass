...
// The characters a masked field is drawn with, and nothing else. Every one of
// these is a *rendering* of a hidden character rather than a character anyone
// types on purpose, which is what makes an all-of-these value safe to refuse.
// Letters are deliberately absent: "xxxxxxxx" is a bad password, not a mask,
// and refusing it would be this rule guessing at quality instead of at kind.
const MASKED = /^[*\u2022\u25CF\u25CB\u25E6\u2219\u00B7\u2027\u2043\u25AA\u25A0]+$/;
 ...
 * more safely than it can be stored here, and the entry can still be written
 * by hand, so the trade is the same one plausibleUsername makes.
 *
 * Empty is not this rule's business — handleCapture has already returned by
 * then — and is answered `false` only because a capture with no password is
 * not a capture. See plausibleUsername for the username half.
 */
export function plausiblePassword(value) {
  const v = String(value ?? '').trim();
  if (!v) return false;
  return !MASKED.test(v);
}
